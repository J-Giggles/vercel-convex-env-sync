...
/**
 * Diff two env maps treating empty-string values the same as missing keys
 * (so `KEY=` locally with no remote entry is not flagged as drift).
 *
...
function diffMaps(local, remote) {
  ...
  const remoteOnly = [];
  const differs = [];
  const all = new Set([...local.keys(), ...remote.keys()]);
  for (const k of all) {
    const lv = local.get(k);
    const rv = remote.get(k);
    const lEmpty = lv === undefined || lv === "";
    const rEmpty = rv === undefined || rv === "";
    if (lEmpty && rEmpty) continue;
    if (lEmpty) remoteOnly.push(k);
    else if (rEmpty) localOnly.push(k);
    else if (lv !== rv) differs.push(k);
  }
  localOnly.sort();
  remoteOnly.sort();
  differs.sort();
...
export async function checkTarget(target, opts = {}) {
  ...
  /** Convex disabled: collapse mode to vercel-only regardless of CLI flags. */
  const convexOnly = opts.convexOnly === true && convexEnabled;
  const vercelOnly = opts.vercelOnly === true || !convexEnabled;
  if (opts.convexOnly && opts.vercelOnly) {
    throw new Error(
      "[env:sync] check: --convex-only and --vercel-only are mutually exclusive."
    );
  }
  if (opts.convexOnly && !convexEnabled) {
    throw new Error(
      "[env:sync] check: --convex-only requested but Convex is disabled (ENV_SYNC_DISABLE_CONVEX=1)."
    );
  }
  ...
  if (!quiet) {
    ...
    if (!convexEnabled) {
      syncDim("  Convex disabled (ENV_SYNC_DISABLE_CONVEX=1) — comparing Vercel only.");
    } else if (fromSync) {
  ...
  const schemaAudit = auditAgainstEnvTs(
    localMap,
    remoteConvex,
    remoteVercelUserManaged
  );
  if (schemaAudit) {
    printSchemaAudit(schemaAudit);
  }
...
/**
 * Infrastructure / build-system keys that env.ts is not expected to validate.
 * Keep narrow — anything project-specific should be in env.ts.
 */
const ENV_TS_AUDIT_IGNORE = new Set([
  "NODE_ENV",
  "CONVEX_DEPLOY_KEY",
  "CONVEX_DEPLOYMENT",
  "CONVEX_URL",
...
/**
 * Cross-check observed keys (local file + hosted Convex/Vercel) against the
 * project's `env.ts` zod schemas + `.env.example` template.
 *
 * @param {Map<string, string>} localMap
 * @param {Map<string, string> | null} remoteConvex
 * @param {Map<string, string> | null} remoteVercel
 * @returns {{
 *   envTsPath: string;
 *   notInEnvTs: { key: string; sources: string[] }[];
 *   missingFromExample: string[];
 *   missingFromEnvTs: string[];
 *   exampleAvailable: boolean;
 * } | null}
 */
function auditAgainstEnvTs(localMap, remoteConvex, remoteVercel) {
  const envTsKeys = loadEnvTsKeys();
  if (!envTsKeys) return null;
  const envTsPath = resolveEnvTsPath();
  const exampleKeys = loadEnvExampleKeys();

  /** @type {Map<string, Set<string>>} */
  const observedSources = new Map();
  const recordSource = (key, source) => {
    if (ENV_TS_AUDIT_IGNORE.has(key)) return;
    if (isVercelAutoInjectedKey(key)) return;
    if (!observedSources.has(key)) observedSources.set(key, new Set());
    observedSources.get(key).add(source);
  };
  for (const [k, v] of localMap) {
    if (v !== "") recordSource(k, "local");
  }
  if (remoteConvex) {
    for (const [k, v] of remoteConvex) {
      if (v !== "") recordSource(k, "convex");
    }
  }
  if (remoteVercel) {
    for (const [k, v] of remoteVercel) {
      if (v !== "") recordSource(k, "vercel");
    }
  }

  /** @type {{ key: string; sources: string[] }[]} */
  const notInEnvTs = [];
  for (const [key, sources] of observedSources) {
    if (!envTsKeys.has(key)) {
      notInEnvTs.push({ key, sources: [...sources].sort() });
    }
  }
  notInEnvTs.sort((a, b) => a.key.localeCompare(b.key));

  /** @type {string[]} */
  const missingFromExample = [];
  /** @type {string[]} */
  const missingFromEnvTs = [];
  if (exampleKeys) {
    for (const k of envTsKeys) {
      if (!exampleKeys.has(k)) missingFromExample.push(k);
    }
    for (const k of exampleKeys) {
      if (!envTsKeys.has(k) && !ENV_TS_AUDIT_IGNORE.has(k))
        missingFromEnvTs.push(k);
    }
    missingFromExample.sort();
    missingFromEnvTs.sort();
  }

  return {
    envTsPath: envTsPath ?? "env.ts",
    notInEnvTs,
    missingFromExample,
    missingFromEnvTs,
    exampleAvailable: exampleKeys !== null,
  };
}

/**
 * @param {NonNullable<ReturnType<typeof auditAgainstEnvTs>>} audit
 */
function printSchemaAudit(audit) {
  const totalIssues =
    audit.notInEnvTs.length +
    audit.missingFromExample.length +
    audit.missingFromEnvTs.length;
  if (totalIssues === 0) return;

  console.log("");
  console.log(
    `${ANSI.bold}Schema audit${ANSI.reset} ${ANSI.dim}· vs ${path.relative(REPO_ROOT, audit.envTsPath)}${ANSI.reset}`
  );

  if (audit.notInEnvTs.length > 0) {
    console.log(
      `  ${ANSI.yellow}${audit.notInEnvTs.length}${ANSI.reset} key(s) present but not validated by env.ts (deprecated or unmanaged)`
    );
    const keyWidth = Math.max(
      "Key".length,
      ...audit.notInEnvTs.map((r) => r.key.length)
    );
    console.log(
      `  ${pad(`${ANSI.dim}Key${ANSI.reset}`, keyWidth)}  ${ANSI.dim}Seen on${ANSI.reset}`
    );
    for (const r of audit.notInEnvTs) {
      console.log(`  ${pad(r.key, keyWidth)}  ${ANSI.dim}${r.sources.join(", ")}${ANSI.reset}`);
    }
  }
  if (audit.missingFromExample.length > 0) {
    console.log("");
    console.log(
      `  ${ANSI.yellow}${audit.missingFromExample.length}${ANSI.reset} key(s) validated by env.ts but missing from .env.example`
    );
    for (const k of audit.missingFromExample) {
      console.log(`    ${k}`);
    }
  }
  if (audit.missingFromEnvTs.length > 0) {
    console.log("");
    console.log(
      `  ${ANSI.yellow}${audit.missingFromEnvTs.length}${ANSI.reset} key(s) in .env.example but not validated by env.ts (deprecated?)`
    );
    for (const k of audit.missingFromEnvTs) {
      console.log(`    ${k}`);
    }
  }
}
