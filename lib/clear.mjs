...
export async function interactiveClear(opts = {}) {
  ...
  try {
    ...
    syncDim("Local `.env*` files are not modified. This only affects Vercel / Convex hosting.");
    if (dryRun) {
      syncWarn("DRY RUN — no variables will be removed.");
    }

    console.log(`
Enter numbers 1–5 separated by commas or spaces (e.g. 1,3  or  2 4):
  1) ${DESTINATIONS[0].label}
  2) ${DESTINATIONS[1].label}
  3) ${DESTINATIONS[2].label}
  4) ${DESTINATIONS[3].label}
  5) ${DESTINATIONS[4].label}
`);
    ...
    /** @type {TClearDestination[]} */
    const selected = uniq.map((n) => DESTINATIONS[n - 1]);
