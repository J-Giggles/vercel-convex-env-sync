 ...
 * by comparing deployment slugs (URL host, deploy key prefix, or legacy CONVEX_DEPLOYMENT).
 */
...
export function extractConvexDeploymentSlug(map) {
  ...
  const dkHead = map.get("CONVEX_DEPLOY_KEY");
  if (typeof dkHead === "string" && dkHead.trim()) {
    const beforePipe = dkHead.split("|")[0].trim();
    const fromKey = slugFromConvexDeploymentVar(beforePipe);
