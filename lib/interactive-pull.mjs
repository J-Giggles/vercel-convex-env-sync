...
import { chooseConvexUseProd, chooseIndex } from "./prompt.mjs";
...
export async function interactivePull(opts = {}) {
  ...
  console.log("[env:sync] Interactive pull — loading Vercel project env inventory…");
  ...
  console.log(
    `[env:sync] Project defines ${envs.length} variable(s) across target(s): ${vercelTargets.join(", ")}`
  );

  console.log("[env:sync] Loading Convex env for development and production deployments…");
  ...
  for (const envName of vercelTargets) {
    console.log(`[env:sync] Pulling Vercel env: ${envName}…`);
  ...
  if (hasUniformAssumedProd) {
    ...
    if (uniformSlug !== null) {
      ...
      console.log(`    (slug "${uniformSlug}" from NEXT_PUBLIC_CONVEX_URL, CONVEX_DEPLOYMENT, etc.).`);
    } else {
      console.log(
  ...
  const idx = await chooseIndex(
    [
      "Choose which Vercel **environment** to merge.",
      "",
      "  This only selects which `vercel env pull --environment …` snapshot is merged.",
      hasUniformAssumedProd
        ? "  Convex variables in the merge come from `convex env list --prod` (see box above)."
        : "  Convex variables follow the inference shown in the summary column for each row.",
      "",
    ].join("\n"),
    rows,
  ...
  if (inf.status === "linked-dev") {
    ...
    console.log(
      "[env:sync] Vercel slug matches both Convex dev and prod maps — choose which Convex deployment to use."
    );
    ...
    const reason =
      inf.status === "no-convex-on-vercel"
        ? "This Vercel environment has no Convex URL / CONVEX_DEPLOYMENT variables (or they are empty)."
        : inf.status === "no-slug"
    ...
    console.log(`[env:sync] ${reason}`);
    useProd = await chooseConvexUseProd(
      "Choose which Convex deployment to merge with this Vercel environment:"
    );
  }

  const convexMap = useProd ? convexProdMap : convexDevMap;
