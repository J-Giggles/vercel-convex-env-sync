...
/**
 * Like {@link chooseIndex} but option **0** pulls every Vercel target (same as `env:sync:pull -- --all`).
 *
 * @template T
 * @param {string} header
 * @param {T[]} items
 * @param {(item: T, index: number) => string} formatLine
 * @returns {Promise<number>} **-1** = pull all; otherwise 0-based index into `items`
 */
export async function choosePullMenuIndex(header, items, formatLine) {
  if (items.length === 0) {
    throw new Error("choosePullMenuIndex: no items");
  }
  const rl = readline.createInterface({ input, output });
  try {
    console.log(header);
    console.log(
      "  0) Pull **all** Vercel targets (development, preview, production) → `.env.sync.*` — same as `pnpm run env:sync:pull -- --all`"
    );
    items.forEach((item, i) => {
      console.log(`  ${i + 1}) ${formatLine(item, i)}`);
    });
    for (;;) {
      const raw = (await rl.question(`Enter 0–${items.length}: `)).trim();
      const n = Number.parseInt(raw, 10);
      if (raw === "0" || n === 0) {
        return -1;
      }
      ...
      console.log(`Invalid choice. Type a number from 0 to ${items.length}.`);
    }
  } finally {
    rl.close();
  }
