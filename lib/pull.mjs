...
export async function executePull(opts) {
  ...
  if (!snapshotOnly && localWrite) {
    ...
    if (templateAbs) {
      try {
        ...
        console.log(
          `[env:sync] Working file layout from template → ${path.relative(REPO_ROOT, templateAbs)}`
        );
      } catch (e) {
        console.warn(
    ...
    if (fs.existsSync(localAbs)) {
      ...
      console.log(
        `[env:sync] Previous ${localRel} saved → ${path.relative(REPO_ROOT, backupPath)}`
      );
    }
    fs.writeFileSync(localAbs, localBody, "utf8");
    console.log(
      `[env:sync] Wrote working env → ${localRel}${created ? " (created)" : ""}`
    );
  }
  ...
  console.log(
    `[env:sync] Wrote merged snapshot → ${path.relative(REPO_ROOT, mergedPath)}`
  );
  console.log(
    `[env:sync] Snapshot hashes — Convex: ${convexHash.slice(0, 12)}… Vercel: ${vercelHash.slice(0, 12)}…`
  );

  if (!prevExists) {
    console.log(
      `[env:sync] No previous merged file — first pull for this key (${label}).`
    );
    ...
    console.log(
      `[env:sync] Merged content changed (${changes.length} key difference(s)):`
    );
