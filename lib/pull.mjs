 ...
 *
 * When Convex is disabled (`ENV_SYNC_DISABLE_CONVEX=1`) only Vercel is pulled and the
 * Convex side becomes an empty map — the merge then equals Vercel.
 */
...
export async function pullTarget(target, opts = {}) {
  ...
  syncInfo(
    convexEnabled
      ? `Pulling Convex + Vercel for target "${target}"…`
      : `Pulling Vercel for target "${target}" (Convex disabled — ENV_SYNC_DISABLE_CONVEX=1).`
  );

  const vercelMap = fetchVercelEnvMap(target);
  /**
   * Convex disabled: skip both Convex CLI calls. Pass an empty map through the merge so
   * the snapshot equals the Vercel pull.
   */
  const convexMap = convexEnabled
    ? resolveConvexMapForVercelPull(
        vercelMap,
        target,
        fetchConvexEnvMap("dev"),
        fetchConvexEnvMap("prod")
      )
    : new Map();
