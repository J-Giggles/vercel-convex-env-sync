...
import { REPO_ROOT, SYNC_DIR } from "./paths.mjs";
 ...
 * @returns {string | null}
 */
function resolveLocalSourcePathSync(target) {
  const candidates =
    target === "dev"
...
export async function pushTarget(target) {
  const localPath = resolveLocalSourcePathSync(target);
  if (!localPath) {
    throw new Error(
  ...
  const convexFile = path.join(SYNC_DIR, `push.convex.${target}.env`);
